package fr.mds.langageobjet.niv2.liste;
...
import fr.mds.langageobjet.niv2.liste.entities.User;
import fr.mds.langageobjet.niv2.liste.utils.ItemsGenerator;
import fr.mds.langageobjet.niv2.liste.entities.Car;
import fr.mds.langageobjet.niv2.liste.entities.CarType;
...
//        nbUsersFullname1316));
    ...
    // 5 : Les utilisateurs possédant une voiture de la marque "marque1"
//    List<User> usersCarMarque1 = users.stream().filter(x -> x.getCar().getCarType().getMark().contains("marque1")).collect(Collectors.toList());
...
//    }
    ...
    // 6 : La première voiture avec le plus haut kilométrage
//    Comparator<Car> comparator = Comparator.comparing( Car::getMileage );
//    Car carMaxMileage = users.stream().max(comparator).get();
...
//    List<Car> carMaxMileages = users.stream().filter(x -> x.getCar().getMileage()).collect(Collectors.toList());
